...
import { Send, Plane, Hotel, Calendar, MapPin, Download, X, ChevronLeft, ChevronRight, ChevronDown, ChevronUp, ExternalLink, CheckCircle, Share2 } from 'lucide-react';
...
export default function ViaggioApp() {
  ...
  const [messages, setMessages] = useState([
    { role: 'assistant', content: "Hello! I'm your Viaggio travel expert. I'm excited to help you plan your perfect trip! Where would you like to go?" }
  ]);
  ...
  // Cart now supports multiple items of each type
  const [cart, setCart] = useState({
    flights: [],    // Changed from single flight to array
    hotels: [],     // Changed from single hotel to array
    tours: []
  });
  ...
  const generateResponse = (userInput) => {
    ...
    if (lower.includes('tour') || lower.includes('activity') || lower.includes('things to do') || lower.includes('experience')) {
      response = "Great! Here are some amazing tours and experiences in Florence:";
      options = travelDatabase.destinations.florence.tours.map(t => ({ type: 'tour', data: t }));
    }
    else if (lower.includes('hotel') || lower.includes('stay') || lower.includes('accommodation')) {
      response = "Excellent! Now let's find you a perfect hotel in Florence's historic center:";
      options = travelDatabase.destinations.florence.hotels.map(h => ({ type: 'hotel', data: h }));
    }
    else if (lower.includes('flight') || lower.includes('fly')) {
      response = "Let me show you some great flight options:";
      options = travelDatabase.destinations.florence.flights.map(f => ({ type: 'flight', data: f }));
    }
    // Extract destination
    else if (lower.includes('florence') || lower.includes('italy')) {
      setConversationContext(prev => ({ ...prev, destination: 'florence' }));
      response = "Wonderful choice! Florence in September is absolutely magical. Let me show you some great flight options:";
      options = travelDatabase.destinations.florence.flights.map(f => ({ type: 'flight', data: f }));
    }
    // Smart suggestions based on what's in cart
    else if (cart.flights.length > 0 && cart.hotels.length === 0) {
      response = "Great flight selections! Now let's find you some hotels. Would you like to see hotel options?";
      options = travelDatabase.destinations.florence.hotels.map(h => ({ type: 'hotel', data: h }));
    }
    else if (cart.hotels.length > 0 && cart.tours.length === 0) {
      response = "Perfect! You have your accommodations sorted. Now let's add some amazing experiences. Here are some top tours in Florence:";
      options = travelDatabase.destinations.florence.tours.map(t => ({ type: 'tour', data: t }));
    }
    else if (lower.includes('itinerary') || lower.includes('final')) {
      setShowItinerary(true);
      response = "Perfect! I've put together your complete Florence itinerary. Click the button to review everything!";
    }
    else {
      response = "I'd be happy to help you plan your trip! You can ask me for:\n\n• Flights to Florence\n• Hotels in Florence\n• Tours and activities\n\nOr just say 'I want to go to Florence, Italy'";
    }
    
    return { role: 'assistant', content: response, options: options };
  };
  ...
  const addToCart = (type, item) => {
    if (type === 'flight') {
      setCart(prev => ({ 
        ...prev, 
        flights: prev.flights.find(f => f.id === item.id) ? prev.flights : [...prev.flights, item]
      }));
    } else if (type === 'hotel') {
      setCart(prev => ({ 
        ...prev, 
        hotels: prev.hotels.find(h => h.id === item.id) ? prev.hotels : [...prev.hotels, item]
      }));
    } else if (type === 'tour') {
      setCart(prev => ({ 
        ...prev, 
        tours: prev.tours.find(t => t.id === item.id) ? prev.tours : [...prev.tours, item]
      }));
    }
  };
  ...
  const removeFromCart = (type, itemId) => {
    if (type === 'flight') {
      setCart(prev => ({ ...prev, flights: prev.flights.filter(f => f.id !== itemId) }));
    } else if (type === 'hotel') {
      setCart(prev => ({ ...prev, hotels: prev.hotels.filter(h => h.id !== itemId) }));
    } else if (type === 'tour') {
      setCart(prev => ({ ...prev, tours: prev.tours.filter(t => t.id !== itemId) }));
    }
  };
        ...
        <div className="max-w-6xl mx-auto px-4 py-8">
          <div className="bg-green-50 border border-green-200 rounded-lg p-6 mb-8">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h2 className="text-xl font-bold text-green-900 mb-2">Your Itinerary is Ready!</h2>
                <p className="text-green-700">Click the booking links below to complete your reservations.</p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            {cart.hotels.map(hotel => (
              <div key={hotel.id} className="bg-white rounded-lg shadow-md border border-gray-200 p-6">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-start gap-3">
                  ...
                  <p className="text-2xl font-bold text-purple-600">{formatCurrency(hotel.price)}</p>
                </div>
                <a
                  href={hotel.link}
                  target="_blank"
            ...
            {cart.tours.map(tour => (
              <div key={tour.id} className="bg-white rounded-lg shadow-md border border-gray-200 p-6">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <h4 className="font-bold text-gray-900 text-lg">{tour.name}</h4>
                      <p className="text-sm text-gray-600 mt-1">{tour.date} at {tour.time}</p>
                    </div>
                  </div>
                  <p className="text-2xl font-bold text-green-600">{formatCurrency(tour.price * 2)}</p>
                </div>
                <a
                  href={tour.link}
                  target="_blank"
    ...
    <div className="flex h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50">
      ...
      {sidebarOpen && (
        <div className="w-80 bg-white border-r border-gray-200 shadow-lg flex flex-col">
          <div className="p-4 border-b border-gray-200 bg-gradient-to-r from-blue-600 to-purple-600 flex items-center justify-between">
            ...
            <button onClick={() => setSidebarOpen(false)} className="p-1 hover:bg-white hover:bg-opacity-20 rounded transition-colors">
              ...
              <>
                ...
                <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                  ...
                  {expandedSections.flights && cart.flights.length > 0 && (
                    <div className="p-3 space-y-2">
                      {cart.flights.map(flight => (
                        <div key={flight.id} className="bg-blue-50 rounded-lg p-3 border border-blue-200">
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              ...
                              <p className="text-xs text-gray-500 mt-1">{flight.departure}</p>
                ...
                <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                  ...
                  {expandedSections.hotels && cart.hotels.length > 0 && (
                    <div className="p-3 space-y-2">
                      {cart.hotels.map(hotel => (
                        <div key={hotel.id} className="bg-purple-50 rounded-lg p-3 border border-purple-200">
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              <p className="text-sm font-semibold text-gray-900">{hotel.name}</p>
                              <p className="text-xs text-gray-600">{hotel.location}</p>
                              <p className="text-xs text-gray-500 mt-1">⭐ {hotel.rating}/5</p>
                ...
                <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                  ...
                  {expandedSections.tours && cart.tours.length > 0 && (
                    <div className="p-3 space-y-2">
                      {cart.tours.map(tour => (
                        <div key={tour.id} className="bg-green-50 rounded-lg p-3 border border-green-200">
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              <p className="text-sm font-semibold text-gray-900">{tour.name}</p>
                              <p className="text-xs text-gray-600">{tour.date} at {tour.time}</p>
      ...
      {!sidebarOpen && (
        <button
          ...
          className="fixed left-0 top-20 bg-blue-600 text-white p-2 rounded-r-lg shadow-lg hover:bg-blue-700 transition-colors z-40"
        >
          ...
          <div className="max-w-3xl mx-auto space-y-4">
            {messages.map((msg, idx) => (
              <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-2xl ${msg.role === 'user' ? 'bg-blue-600 text-white rounded-2xl px-5 py-3' : ''}`}>
                  {msg.role === 'assistant' && (
                      ...
                      {msg.options && msg.options.length > 0 && (
                        <div className="space-y-3">
                          {msg.options.map((option, optIdx) => {
                            const isSelected = isInCart(option.type, option.data.id);
                            const colors = {
                              flight: { border: 'border-blue-600', bg: 'bg-blue-50', icon: 'text-blue-600', button: 'bg-blue-600 hover:bg-blue-700' },
                              hotel: { border: 'border-purple-600', bg: 'bg-purple-50', icon: 'text-purple-600', button: 'bg-purple-600 hover:bg-purple-700' },
                              tour: { border: 'border-green-600', bg: 'bg-green-50', icon: 'text-green-600', button: 'bg-green-600 hover:bg-green-700' }
                            }[option.type];
                            
                            return (
                              <div key={optIdx} className={`bg-white rounded-lg shadow-md border-2 p-4 transition-all ${isSelected ? colors.border + ' ' + colors.bg : 'border-gray-200'}`}>
                                <div className="flex items-start justify-between mb-3">
                                  <div className="flex items-start gap-3 flex-1">
                                    {option.type === 'flight' && <Plane className={`w-5 h-5 ${colors.icon} mt-1`} />}
                                    {option.type === 'hotel' && <Hotel className={`w-5 h-5 ${colors.icon} mt-1`} />}
                                    {option.type === 'tour' && <MapPin className={`w-5 h-5 ${colors.icon} mt-1`} />}
                                    <div className="flex-1">
                                      <h4 className="font-bold text-gray-900">{option.data.airline || option.data.name}</h4>
                                      {option.data.route && <p className="text-sm text-gray-600">{option.data.route}</p>}
                                      {option.data.location && <p className="text-sm text-gray-600">⭐ {option.data.rating}/5 | {option.data.location}</p>}
                                      {option.data.date && <p className="text-sm text-gray-600">{option.data.date} at {option.data.time}</p>}
                                    </div>
                                  </div>
                                  <div className="text-right ml-4">
                                    <p className={`text-xl font-bold ${colors.icon}`}>
                                      {formatCurrency(option.type === 'tour' ? option.data.price * 2 : option.data.price)}
                                    </p>
                                  </div>
                                </div>
                                <div className="flex gap-2">
                                  ...
                                  <button
                                    onClick={() => isSelected ? removeFromCart(option.type, option.data.id) : addToCart(option.type, option.data)}
                                  ...
                                  >
                                    {isSelected ? 'Remove' : option.type === 'tour' ? 'Add to Trip' : 'Select'}
                                  </button>
                                </div>
                              </div>
                            );
